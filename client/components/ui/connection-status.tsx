...
export function ConnectionStatus({ showDetails = false, className = "" }: ConnectionStatusProps) {
  ...
  const getStatusIcon = () => {
    ...
    if (isConnecting) return <RefreshCw className="h-4 w-4 text-blue-500 animate-spin" />;
    if (isReconnecting) return <RefreshCw className="h-4 w-4 text-yellow-500 animate-spin" />;
